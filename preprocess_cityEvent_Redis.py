...
r = redis.StrictRedis(host='152.46.19.205', port=6379, db=0)
...
event_rdd = event_df.rdd.map(lambda x: (x.city, Rake.run(x.data)[0:5])).map(extractWords).groupByKey().mapValues(list).mapValues(flatten).mapValues(createDict).mapValues(createSortedList)

# event_rdd_1 = event_rdd
...
event_data = event_rdd.collect()

for event in event_data:
	for item in event[1]:
		r.lpush(event[0], item)
